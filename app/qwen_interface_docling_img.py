...
def pipeline_question_answer(pdf_file, user_question, top_k=3):
    ...
    try:
        ...
        # Vérifier le cache
        cached_data = cache_manager.load_cache(local_pdf)
        if cached_data:
            text_chunks, embeddings, index = cached_data
            ...
            # Extraction markdown + découpe (avec descriptions d’images)
...
demo = gr.Interface(
    ...
    title="Qwen PDF Q&A (Docling-powered with VLM)",
